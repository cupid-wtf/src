...
export async function POST(req: Request) {
    ...
    try {
        ...
        const bannerUrl = `https://r2.opium.bio/${key}`;
        await UserModel.findByIdAndUpdate(session.id, { banner: bannerUrl });

        return new NextResponse(
            JSON.stringify({ success: true, message: "File uploaded successfully", bannerUrl }),
            { status: 200, headers: { 'Content-Type': 'application/json' } }
        );

    } catch (error) {
        return new NextResponse(
            JSON.stringify({ success: false, message: "File upload failed", error }),
...
export async function DELETE() {
    ...
    try {
        ...
        if (!user || !user.banner) {
            return new NextResponse(
                JSON.stringify({ success: false, message: "No banner found to delete" }),
        ...
        await UserModel.findByIdAndUpdate(session.id, { banner: null });
